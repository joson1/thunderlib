...
static vu8  fac_us=22;							//us延时倍乘数			   
static vu16 fac_ms=22500;							//ms延时倍乘数,在os下,代表每个节拍的ms数
...
//对180M条件下,nms<=745ms 
void delay_xms(vu16 nms)
{	 		  	  
	vu32 temp;		   
	SysTick->LOAD=(vu32)nms*fac_ms;			//时间加载(SysTick->LOAD为24bit)
	SysTick->VAL =0x00;           			//清空计数器
	SysTick->CTRL=0x01 ;          			//开始倒数  
	do
	{
...
{	 	 
	vu8 repeat=nms/540;						//这里用540,是考虑到某些客户可能超频使用,
